# item:copper_dagger/use_mainhand
# called by advancement: item:use/copper_dagger

## Summon
execute anchored eyes run summon snowball ^ ^ ^ {Tags:[maddons.copper_dagger]}
execute if entity @s[gamemode=creative] run tag @n[type=snowball,tag=maddons.copper_dagger,tag=!maddons.summoned] add maddons.no_break
...
data modify entity @n[type=snowball,tag=maddons.copper_dagger,tag=!maddons.summoned] Item set from entity @s SelectedItem
