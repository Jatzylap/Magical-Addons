...
execute if entity @s[gamemode=creative] as @n[tag=maddons.iron_dagger,type=snowball,tag=!maddons.summoned] at @s on passengers run tag @s[type=item_display] add maddons.no_break
...
data modify entity @n[type=snowball,tag=maddons.iron_dagger,tag=!maddons.summoned] Item set from entity @s Inventory[{Slot:-106b}]
