# item:wooden_wand/use_offhand
# called by: item:wooden_wand/use

## Use (Creative Mode)
execute if entity @s[gamemode=creative] run function magical_addons:item/wooden_wand/disambiguation {hand:"offhand"}
...
$execute if entity @s[gamemode=!creative] if items entity @s weapon.offhand *[damage=$(max_damage)] run particle item{item:{id:"recovery_compass",components:{item_model:"magical_addons:wooden_wand"}}} ^-.4 ^ ^.1 0 0 0 .2 5 normal @a
...
## Damage
execute if entity @s[gamemode=!creative] run item modify entity @s weapon.offhand magical_addons:item/damage/wooden_wand
