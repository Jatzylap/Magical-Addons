# item:copper_wand/use_mainhand
# called by: item:copper_wand/use

## Use (Creative Mode)
execute if entity @s[gamemode=creative] run function magical_addons:item/copper_wand/disambiguation {hand:"mainhand"}
...
$execute if entity @s[gamemode=!creative] if items entity @s weapon.mainhand *[damage=$(max_damage)] run particle item{item:{id:"recovery_compass",components:{item_model:"magical_addons:copper_wand"}}} ^-.4 ^ ^.1 0 0 0 .2 5 normal @a
...
## Damage
execute if entity @s[gamemode=!creative] run item modify entity @s weapon.mainhand magical_addons:item/damage/copper_wand
