# magical_addons:entity/automaton/animate/punch
...
execute if score @s maddons.cooldown matches 25 on vehicle on vehicle at @s run function magical_addons:entity/automaton/effects/punch_hit
### Damage (moved from punch_hit.mcfunction because it conflicts with player motion)
execute if score @s maddons.cooldown matches 26 on vehicle on vehicle at @s as @e[distance=..5,type=!#magical_addons:invulnerable,tag=!maddons.invul,tag=!maddons.automaton] at @s run damage @s 4.5 mob_attack by @n[tag=maddons.automaton,tag=maddons.vehicle]
