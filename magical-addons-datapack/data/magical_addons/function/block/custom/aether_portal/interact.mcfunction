...
execute if predicate magical_addons:shared/night unless dimension minecraft:the_aether if block ~ ~ ~ #magical_addons:aether_portal{components:{"minecraft:custom_data":{magical_addons:{id:"aether_portal"}}}} run playsound block.portal.trigger block @s ~ ~ ~ 0.5
