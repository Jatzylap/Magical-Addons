# magic:water_spell/loop
# called by: main:tick_entity

## Block
execute positioned ^ ^ ^1 unless block ~ ~ ~ #magical_addons:non_collidable positioned ^ ^ ^-1 run return run function magical_addons:magic/water_spell/hit_block
...
execute if entity @n[type=!#magical_addons:invulnerable,tag=maddons.target_entity] store result score @s maddons.damage run function magical_addons:magic/water_spell/hit_entity
