...
execute as @e[type=armor_stand,tag=maddons.falling_block] at @s if data entity @s {OnGround:1b} run function magical_addons:dev/set_custom_block with entity @s data.magical_addons