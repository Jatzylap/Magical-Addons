...
# @input
#   axis
#       The axis of the current face's normal; one of "x", "y", "z"
# @output
...
$scoreboard players operation $distance iris = $$(axis)0 iris
$scoreboard players operation $distance iris -= ${$(axis)} iris
scoreboard players operation $distance iris *= $1000 iris
$scoreboard players operation $distance iris /= $d$(axis) iris
...
# Override for the current axis
$scoreboard players operation $$(axis)_intersection iris = $$(axis)0 iris
...
# See if that intersection falls within the face, otherwise fail
execute if score $x_intersection iris >= $x0 iris if score $x_intersection iris <= $x1 iris if score $y_intersection iris >= $y0 iris if score $y_intersection iris <= $y1 iris if score $z_intersection iris >= $z0 iris if score $z_intersection iris <= $z1 iris run return run scoreboard players get $distance iris
