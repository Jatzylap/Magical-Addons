...
## Set
data modify storage iris:args command set value "execute align xyz positioned ~.5 ~.5 ~.5 run function magical_addons:block/mine"
...
scoreboard players operation $max_depth iris += #1 maddons.constant
function iris:raycast/loop

## Save
function magical_addons:block/mine_save with storage iris:output