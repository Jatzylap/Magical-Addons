...
## If ON
execute if block ~ ~ ~ barrel[open=true] as @n[type=item_display,tag=maddons.wizarding_table,tag=on,tag=!off,distance=...5] run function magical_addons:dev/register_entity {cmd:"function magical_addons:block/custom/wizarding_table/tick"}