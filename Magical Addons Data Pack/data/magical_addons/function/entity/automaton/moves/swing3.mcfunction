...
## Set cooldown frame
execute store result score #maddons.temp maddons.temp run scoreboard players set @s maddons.cooldown 160

## Set animation
execute on passengers run data modify entity @s[tag=maddons.head] item.components."minecraft:custom_model_data".floats[1] set value 5f
data modify entity @s item.components."minecraft:custom_model_data".floats[1] set value 5f
data modify entity @s item.components."minecraft:custom_data".magical_addons merge value {attack:"swing3"}
...
scoreboard players remove @s maddons.animation 159
