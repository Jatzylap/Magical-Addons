...
execute store result block ~ ~ ~ components."minecraft:custom_data".magical_addons.damage int 1 store result score @s maddons.temp run data get entity @p[advancements={magical_addons:---/block/interact/wizarding_table_off=true}] SelectedItem{components:{"minecraft:custom_data":{magical_addons:{id:"sparkstone_orb"}}}}.components."minecraft:damage"
