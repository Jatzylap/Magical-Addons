...
## Block
execute positioned ^ ^ ^1 unless block ~ ~ ~ #magical_addons:air_like positioned ^ ^ ^-1 run function magical_addons:item/baton/hit_block

## Entity
tag @n[type=!#magical_addons:invulnerable,tag=!maddons.invul,distance=..2] add maddons.spell.hit
...
execute positioned ^ ^ ^1 if score @s maddons.temp matches 1 run function magical_addons:item/baton/hit_entity
