...
$execute if entity @s[gamemode=!creative] if score @s maddons.mana matches $(min_mana).. if items entity @s weapon.offhand *[damage=$(max_damage)] run particle item{item:{id:"recovery_compass",components:{item_model:"magical_addons:baton"}}} ^.4 ^ ^.1 0 0 0 .2 5 normal @a
...
## Damage
$execute if entity @s[gamemode=!creative] if score @s maddons.mana matches $(min_mana).. run item modify entity @s weapon.offhand magical_addons:item/damage/baton
