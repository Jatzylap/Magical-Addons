...
$execute if entity @s[gamemode=!creative] if score @s maddons.mana matches $(min_mana).. if items entity @s weapon.mainhand *[damage=$(max_damage)] run particle item{item:{id:"warped_fungus_on_a_stick",components:{item_model:"magical_addons:$(id)"}}} ^-.4 ^ ^.1 0 0 0 .2 5 normal @a
...
## Damage
$execute if entity @s[gamemode=!creative] if score @s maddons.mana matches $(min_mana).. run item modify entity @s weapon.mainhand magical_addons:item/damage/$(id)
