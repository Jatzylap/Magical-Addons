# item:dagger/wooden_dagger/break
# called by: item:dagger/wooden_dagger/loop

execute if entity @s[tag=!maddons.no_break] run loot spawn ~ ~ ~ loot magical_addons:items/wooden_dagger
execute if entity @s[tag=!maddons.no_break] run item replace entity @n[type=item,dy=1,nbt={Age:0s}] contents from entity @s contents magical_addons:item/damage/wooden_dagger
...
particle minecraft:item{item:{id:"snowball",components:{item_model:"magical_addons:wooden_dagger"}}} ~ ~.1 ~ 0 0 0 .1 9
