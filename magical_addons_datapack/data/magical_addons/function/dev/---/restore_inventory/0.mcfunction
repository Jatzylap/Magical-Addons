...
$execute in magical_addons:---/void run data modify block 0 0 0 Items set from storage magical_addons:player data.inventories[{UUID:$(UUID)}].inventory$(slot).section1
$execute in magical_addons:---/void run data modify block 0 1 0 Items set from storage magical_addons:player data.inventories[{UUID:$(UUID)}].inventory$(slot).section2
