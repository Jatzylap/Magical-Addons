...
## Move
execute store success entity @s item.components."minecraft:custom_model_data".floats[1] float 1 run data modify entity @s item.components."minecraft:custom_data".magical_addons.pos set from entity @s Pos

## Rotate Body
execute on vehicle at @s on passengers run rotate @s[tag=maddons.body] ~ 0

## Rotate head
execute unless score @s maddons.cooldown matches 0.. on vehicle at @s on passengers if entity @s[tag=maddons.entity] on passengers facing entity @p[tag=!maddons.invul,distance=..64] eyes run return run rotate @s[tag=maddons.head] ~ 0
