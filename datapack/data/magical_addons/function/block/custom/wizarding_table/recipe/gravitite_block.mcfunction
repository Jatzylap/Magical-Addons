# block:custom/wizarding_table/recipe/gravitite_block
...
execute store success score @s maddons.craft if items block ~ ~ ~ container.2 *[custom_data~{magical_addons:{id:"gravitite"}}] \
											 ...
											 if items block ~ ~ ~ container.22 *[custom_data~{magical_addons:{id:"gravitite"}}] run loot replace block ~ ~ ~ container.16 loot magical_addons:gravitite_block
