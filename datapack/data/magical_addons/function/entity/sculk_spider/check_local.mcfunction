...
## Damage
execute if entity @s[tag=maddons.entity.ai,tag=!maddons.entity.death] on vehicle on vehicle anchored eyes positioned ^ ^ ^0.25 run damage @n[tag=!maddons.sculk_spider,type=!#magical_addons:invulnerable,distance=..1.5] 3.0 mob_attack by @s

## Drop over vulnerable entity
execute on vehicle on vehicle if entity @n[type=!#magical_addons:invulnerable,tag=!maddons.invul,dy=-32] run data merge entity @s {NoGravity:0b}